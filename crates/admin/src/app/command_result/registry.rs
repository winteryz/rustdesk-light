...
mod tests {
    ...
    #[test]
    fn rows_group_by_canonical_hive_and_path() {
        let table = ResultTable {
            headers: strings(["Hive", "Path", "Name", "Type", "Value"]),
            rows: vec![
                strings(["HKCU", r"Software\Run", "One", "String", "a.exe"]),
                strings([
                    "HKEY_CURRENT_USER",
                    r"Software\Run",
                    "Two",
                ...
                strings(["HKLM", r"Software\RunOnce", "Three", "DWord", "1"]),
            ],
        };

        let groups = registry_groups(&table, "");

        assert_eq!(groups.len(), 2);
        assert_eq!(groups[0].hive, "HKEY_CURRENT_USER");
        assert_eq!(groups[0].path, r"Software\Run");
        assert_eq!(groups[0].rows.len(), 2);
        assert_eq!(groups[1].hive, "HKEY_LOCAL_MACHINE");
        assert_eq!(groups[1].rows.len(), 1);
    }

    #[test]
    fn display_paths_use_full_root_names() {
        assert_eq!(registry_display_hive("HKCU"), "HKEY_CURRENT_USER");
        assert_eq!(
            registry_display_path("HKLM", r"Software\Microsoft"),
            r"HKEY_LOCAL_MACHINE\Software\Microsoft"
        );
    }

    #[test]
    fn key_payload_uses_full_root_name_for_exact_hive_and_path() {
        let payload = registry_list_key_payload("HKEY_LOCAL_MACHINE", r"Software\Microsoft");

        assert!(payload.contains("action=list_key"));
    ...
    #[test]
    fn value_table_filters_key_placeholder_rows() {
        let headers = strings(["Hive", "Path", "Name", "Type", "Value"]);
        let key_row = strings([
            "HKEY_LOCAL_MACHINE",
        ...
        assert!(registry_row_is_key(&headers, &key_row));
        assert!(!registry_row_is_key(&headers, &value_row));
    }

    #[test]
    fn detail_merge_keeps_expanded_keys() {
        let existing = "\
registry_manager:
Hive\tPath\tName\tType\tValue
HKLM\t-\t(key)\tKey\t-
HKLM\tSoftware\t(key)\tKey\t-";
        let incoming = "\
registry_manager:
Hive\tPath\tName\tType\tValue
HKEY_LOCAL_MACHINE\tSoftware\t(key)\tKey\t-
HKEY_LOCAL_MACHINE\tSoftware\tPath\tString\tC:\\App
HKEY_LOCAL_MACHINE\tSoftware\\Microsoft\t(key)\tKey\t-";

        let merged = merge_details(existing, incoming).expect("registry table merges");
        let table = parse_result_table(&merged).expect("merged table stays parseable");

        assert_eq!(table.rows.len(), 4);
        assert!(table.rows.iter().all(|row| row[0] == "HKEY_LOCAL_MACHINE"));
        assert!(table.rows.contains(&strings([
            "HKEY_LOCAL_MACHINE",
            r"Software\Microsoft",
            "(key)",
            "Key",
